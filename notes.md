...
| **Day 11** | **高频错误检测功能**                            | **✅（09-09）**     |
| **Day 12** | **真实脱敏日志测试（发现HTML功能回归等5项）**   | **✅（09-10）**     |
| **Day 13** | **修复Day 12发现的问题（HTML回归+分类覆盖+0行提示）** | **✅（09-12）**     |
| Day 14     | README + 技术总结 + 推送GitHub                  | ⏳ 未开始          |

> **当前状态**：成品1 Day 1-13 完成 ✅。Day 13 已修复 Day 12 发现的 5 项问题（HTML/`--format` 回归恢复并整合高频错误、存储错误分类、数据库超时归类、0行友好提示、HTML空表头优化），9 组测试全过。Plan B 剩余：**Day 14**（README/技术总结/推送/简历/话术/AI模拟面试）。
...
## 十四、Day 11 笔记：错误模式检测（09-09）

### Day 11 新增5个知识点（逐词注释）

1. **错误指纹** = 去掉时间戳后的完整错误文本
   - `' '.join(line.split()[2:])` —— split后用切片`[2:]`跳过前两段（日期+时间），剩余部分用空格拼回
   - 为什么不能拿整行当key？→ 每行时间戳都不同，整行比对手手不同，统计失效

2. **字典 value 结构** = `[次数, 首次时间, 末次时间]`
   - 用列表不用元组 → 列表可原地修改（`info[0] += 1`），元组不可变
   - 首次时间不动（"出生证明"），末次时间随每次出现刷新

3. **数完再筛** vs 边数边判
   - 数完再筛：先把所有错误指纹统计完，再筛选`次数 >= 阈值`的条目
   - 边数边判：循环里遇到第3次就立刻标记 → 数据不完整（后面可能还有第4、5次）
   - 正确：数据收集和业务判断分离

4. **列表遍历** vs 字典遍历
   - `find_frequent_errors` 返回**列表**，每个元素是元组
   - 遍历列表：`for fingerprint, count, first_time, last_time in frequent_errors:`
   - 列表没有`.items()`方法，字典才有

5. **常量职责分离**
   - `CATEGORIES` 字典只负责"错误分类关键词映射"
   - `FREQUENT_THRESHOLD` 是"高频判定阈值"，独立常量
   - 不同职责的东西不要混在一起
...
| 函数 | 职责 | 参数 | 返回 |
|------|------|------|------|
| `find_frequent_errors(lines, threshold=3)` | 找出高频错误（同一错误信息出现次数>=阈值） | lines: 所有日志行<br>threshold: 高频判定阈值（可选，默认3） | 列表，每个元素是元组 `(错误指纹, 次数, 首次时间, 末次时间)` |
...
2. **报告新增章节**（`generate_report` 函数）
   - 高频错误清单插在"级别统计"之前（重点先行）
   - 格式：`出现次数超过3次的错误：` + 每条错误详情

3. **主入口调用**（单文件分支 + 批量分支）
   ```python
   frequent_errors = find_frequent_errors(lines)
   generate_report(..., frequent_errors, args.output)  # 参数顺序：frequent_errors在output_path前
   ```

### Day 11 创阶段增强

- 可加 `--threshold` 参数让用户自定义高频判定阈值（当前写死为3）

### Day 11 最终验证（边界/回归测试全过）
...
**边界测试**：
- [x] 空文件（empty.log）→ 无报错，正常退出
- [x] 异常格式（bad.log）→ 无报错，级别识别为UNKNOWN
- [x] 文件不存在（notexist.log）→ 友好提示，正常退出

**高频检测验证**：
- [x] test_frequent.log（MySQL错误4次，Redis错误2次）→ 只检测出MySQL错误

### Day 11 踩坑记录（按发生顺序）

1. **混淆"错误类别"和"错误指纹"** → `classify_errors`按类别归类，`find_frequent_errors`按指纹归类，职责不同
2. **把常量放进字典** → `FREQUENT_THRESHOLD` 错误地放在 `CATEGORIES` 字典里，应独立为全局常量
3. **用`.items()`遍历列表** → 列表没有这个方法，应直接遍历
4. **参数顺序写反** → `generate_report(..., args.output, frequent_errors)` 应改为 `generate_report(..., frequent_errors, args.output)`
5. **报告重复写高频错误** → 同一章节写了两次，应只保留一处

### Day 11 收尾四问

1. **今天产出了什么？** → 新增 `find_frequent_errors` 函数（高频错误检测）+ 报告新增"高频错误清单"章节 + 新增全局常量 `FREQUENT_THRESHOLD = 3`
2. **跑通了吗？** → 回归测试3/3全过 + 边界测试3/3全过 + 高频检测验证通过
3. **卡在哪了？** → ① 混淆"错误类别"和"错误指纹" ② 把常量放进字典 ③ 用`.items()`遍历列表 ④ 参数顺序写反 ⑤ 报告重复写高频错误
4. **到布卢姆第几层了？** → 记忆层（默写函数签名）✅ / 理解层（为什么用列表存value）✅ / 应用层（跑通测试用例）✅ / 分析层（为什么数完再筛）✅ / 评价层（高频清单放报告什么位置）✅ / 创造层（提出`--threshold`参数增强）✅

### 一句话说清今天最重要的概念

> 高频错误检测 = **去时间戳提取错误指纹** → **字典统计次数/首次/末次** → **筛选阈值以上条目** → **报告优先展示**。核心是"数完再筛"——数据收集和业务判断分离。

### Git 提交

```
Day 11: 错误模式检测（高频错误清单）（1952139）
```

***

## 十五、Day 12 笔记：真实脱敏日志测试（09-10）

### 脚本一句话

Day 12 不改代码，只做一件事：**用一份真实脱敏日志跑遍现有全部功能路径，把发现的问题记成清单交给 Day 13**。

### 本次测试对象

| 项 | 路径 |
|----|------|
| 原始日志（本地，不入Git） | `E:\新的学习\练习\realistic_sample.log` |
| 脱敏映射表 | `E:\新的学习\练习\日志脱敏映射表.txt` |
| 脱敏后日志（本地，不入Git） | `E:\新的学习\练习\realistic_sample_1.log` |
| 单文件报告 | `E:\新的学习\练习\day12_report.txt` |
| 批量报告 | `E:\新的学习\练习\day12_batch_report.txt` |
| 时间过滤报告 | `E:\新的学习\练习\day12_filter_report.txt` |
| 0行边界报告 | `E:\新的学习\练习\day12_filter_empty.txt` |

### Day 12 新增4个知识点

1. **脱敏按"类别"查，不按"感觉"查**
   - 敏感类别：IP / 用户名 / 主机名 / 系统名 / 路径 / 域名 / 任务名
   - 只改IP不够——主机名+系统名+路径组合起来也能反推出真实环境

2. **同一真实值必须"一致映射"**
   - `desktop-fin-01` 在日志中出现3次，必须都替成同一个 `host-01`
   - 不一致会破坏重复关系，让统计/高频分析失真

3. **脱敏要保留"格式类型"和"测试价值"**
   - 主机名替成像主机名的、用户名替成像用户名的
   - 时间/级别/错误关键词尽量保留，否则脚本测不出真实效果

4. **"跑通" ≠ "没bug"——功能路径要逐条验证**
   - 默认txt路径通过，不代表html路径也在
   - 回归bug最爱藏在"没测到的分支"里（本次就是靠换路径才发现）

### Day 12 发现的5项问题（Day 13 输入清单）

| # | 问题 | 分级 | 证据 | 复现命令 |
|---|------|------|------|----------|
| 1 | **HTML报告/`--format`功能回归丢失** | 🔴真bug·必修 | Git: `ae796d2`(Day10)新增 → `1952139`(Day11)删除 | `python log_analyzer.py --file xxx.log --format html` |
| 2 | 存储类错误落入"其他错误" | 🟡覆盖不全·可修 | 第18行 storage写文件失败，`CATEGORIES`无storage/write关键词 | `--file realistic_sample_1.log` |
| 3 | 数据库超时被归成"网络错误" | 🟡覆盖不全·可修 | 第9行同时含connection和MySQL，"首个匹配"先命中网络错误 | 同上 |
| 4 | 时间过滤结果为0行时缺友好提示 | 🟢体验优化·可修 | `day12_filter_empty.txt` 生成空报告，无"该范围内无日志"提示 | `--start 2026-08-02 --end 2026-08-02` |
| 5 | 高频清单为空 | ✅正常现象 | 20行内无重复3次的ERROR，属样本小导致 | — |

### Day 12 额外清理动作

- **删除仓库外旧副本** `E:\新的学习\log_analyzer.py`
  - 原因：存在两份脚本，从 `E:\新的学习>` 直接 `python log_analyzer.py` 跑的是仓库外旧版（更旧，且未纳入Git）
  - 已用 `git ls-files` 确认：根目录那份不在Git跟踪列表，删除安全，不影响GitHub正式版
  - 教训：脚本要从**脚本所在目录**运行，或使用完整路径

### Day 12 验证记录（功能路径全走一遍）

| # | 命令 | 结果 |
|---|------|------|
| 1 | `--file realistic_sample_1.log` | ✅ 通过（INFO8/WARNING5/ERROR7；分类3/2/1/1） |
| 2 | `--dir ...\logs` | ✅ 通过（app1+app2，总11行，错误总计3） |
| 3 | `--file ... --start 2026-08-01 --end 2026-08-01` | ✅ 通过（总行数20，参数生效） |
| 4 | `--file ... --start 2026-08-02 --end 2026-08-02` | ✅ 通过（总行数0，过滤生效） |
| 5 | `--format html` | ❌ **功能不存在（回归丢失）** |

### Day 12 踩坑记录

1. **以为 `--dir` 没扫到练习目录是bug** → 错。`--dir`只扫"传入的那一个目录"，这是职责边界，不是bug
2. **差点当场改HTML回归** → Day 12职责是"记录"，修复留给Day 13；当场改会搅乱问题清单
3. **从错误的工作目录跑脚本** → 根目录`python log_analyzer.py`跑到的是仓库外旧副本，应`cd`进脚本目录

### Day 12 收尾四问

1. **今天产出了什么？** → 脱敏日志1份 + 问题清单5项（含1个真bug回归）+ 清理仓库外旧脚本副本
2. **跑通了吗？** → 4条功能路径通过，1条（html）确认回归丢失
3. **卡在哪了？** → ① 误判`--dir`为bug ② 两份脚本跑错副本 ③ html功能静默消失不易察觉
4. **到布卢姆第几层了？** → 记忆层（脱敏类别）✅ / 理解层（一致映射原理）✅ / 应用层（跑通4条路径）✅ / 分析层（用Git pickaxe定位回归提交）✅ / 评价层（问题分级：真bug/覆盖不全/体验优化/正常）✅ / 创造层（提出用"会筛掉东西"的范围验证过滤逻辑）✅

### 一句话说清今天最重要的概念

> Day 12 的价值不是"脚本能跑"，而是**用真实脱敏日志逐条走功能路径，把藏起来的回归和覆盖盲区暴露成一张可执行的问题清单**——"跑通"只是起点，"逐路径验证 + 分级记录"才是目标。

### Git 提交

```
docs: Day12 真实脱敏测试（问题清单+进度更新）（d8eff3e）
```

***

## 十六、Day 13 笔记：修复 Day 12 发现的 5 项问题（09-12）

### 脚本一句话

Day 13 的核心不是"写新功能"，而是**修复回归 + 合并增量**：把 Day 11 重构时丢失的 HTML / `--format` 功能（对照 Git `ae796d2` 精准补回），补回时把 Day 11 新增的**高频错误**一并整合进 HTML——做到"**修回归不丢新功能**"。

### 本次修复对象

| 项 | 路径 |
|----|------|
| 代码（Git 跟踪，已提交） | `ops-automation-learning\product1-log-analyzer\log_analyzer.py` |
| 真实脱敏日志（本地，不入Git） | `E:\新的学习\练习\realistic_sample_1.log` |

### Day 13 新增知识点

1. **修复回归 ≠ 回退版本，而是"把丢失的增量重新合并进来"**：当前文件被 Day 11 改过，"**既有失（HTML 被删）也有得（新增高频错误）**"，若整体 checkout 回 Day 10，会按下葫芦浮起瓢。正解：`git show ae796d2:...log_analyzer.py` 取出旧版 → 看清它新增了哪几块 → 一块块搬回当前文件
2. **数据层与格式层的职责边界**：`build_*_report_content()` 只 `return` 纯 dict（不写文件、不管 `output_path`），`write_html_report()` 专门把 dict 渲染成 HTML 写文件；加新格式只加 writer，不动数据层
3. **4 列表格 vs 2 列表格不能用同一个渲染函数**：`counter`/`error_counter` 是**字典** `{名:数}`（`.items()` 可迭代），`frequent_errors` 是**列表套 4 元组** `(指纹,次数,首次,末次)`；硬塞进同一个 `_render_stats_table` 会 `AttributeError`。正解：新写 `_render_frequent_errors_table` 专渲染 4 列
4. **关键词映射顺序 = 匹配优先级**：`classify_error` 是"**首个匹配先赢**"，**具体类别**（数据库/存储）必须排在**泛化类别**（网络）前面；关键词**粒度也要够具体**（用 `'write file'` 而非裸词 `'write'`，否则 `[permission] cannot write to` 会被存储类误抢）
5. **业务函数只干自己的事**：`filter_lines_by_time` 只做"lines 进 → lines 出"，"提示用户"是**主入口**的职责；0 行提示因此加在主入口
6. **空列表边界要用 `else` / `continue` 兜住**：单文件分支 0 行时，若不把"计算 + 报告生成"整段包进 `else`，会引用尚未定义的 `counter` → `NameError`；批量分支在 `for` 循环里用 `continue` 更干净
7. **两个格式层行为要对齐**：txt 版无高频时显示提示文案，HTML 版也要（`if not frequent_errors: return "<p>…</p>"`，避免只剩表头的空表格）

### Day 13 修复清单（5 项 → 4 修 + 1 判正常）

| # | 问题 | 修复方式 | 验证结果 |
|---|------|----------|----------|
| 1 | **HTML/`--format` 回归** | 对照 Git `ae796d2` 精准补回：`import html` + 数据层 2 函数（带 `frequent_errors`）+ 格式层 3 函数 + `--format` 参数（`choices=['txt','html']`、`default='txt'`）+ 主入口两条分流 | 单文件 HTML ✅ / 批量 HTML ✅ |
| 2 | 存储类错误落"其他错误" | `CATEGORIES` 新增 `'存储错误': ['storage','write file']` | "其他错误" 1 → 0 ✅ |
| 3 | 数据库超时误归网络错误 | 新增 `'数据库错误': ['MySQL','database']` 置于字典**最前**；`Redis` 保留在服务异常（选 B：`service` 就是服务） | MySQL 归数据库 ✅ |
| 4 | 时间过滤 0 行缺友好提示 | 主入口：单文件用 `else` 包住、批量用 `continue`；提示**含文件名 + 起止日期** | 0 行不崩 + 有提示 ✅ |
| 5 | 高频清单为空 | 判定为**正常现象**（20 行样本无重复 3 次 ERROR），不改；但顺手把 HTML 空表头改为友好段落 | — |

### Day 13 验证记录

| # | 测试场景 | 命令 | 结果 |
|---|----------|------|------|
| 1 | 语法编译 | `python -m py_compile log_analyzer.py` | ✅ 通过 |
| 2 | 单文件 HTML | `--file sample.log --format html` | ✅ 级别+高频+错误分类三节齐全 |
| 3 | 批量 HTML | `--dir .\logs --format html` | ✅ 每文件分段 + 所有文件总计 |
| 4 | 默认 txt（向后兼容） | `--file sample.log`（不传 --format） | ✅ 仍走老函数 |
| 5 | 非法值安检 | `--format bad` | ✅ argparse 自动拒绝，exit=2 |
| 6 | 时间过滤 0 行 | `--start 2026-08-02 --end 2026-08-02` | ✅ 只提示不生成空报告，exit=0 |
| 7 | 高频错误 HTML 渲染 | 自造 t_freq.log（含重复3次错误+`<script>`） | ✅ 4 列数据正确 + `html.escape` 转义生效 |
| 8 | 边界四件套 | 空文件/异常格式/文件不存在/目录不存在 | ✅ 全部 exit=0 不崩 |
| 9 | 真实脱敏日志回归 | `--file realistic_sample_1.log` | ✅ 错误总数守恒（网络2+权限2+数据库1+存储1+服务异常1=7） |

### Day 13 踩坑记录

1. **`build_*` 第一版又写成"直接写文件"** → 职责错位，数据层应只 `return` dict（Day 10 已踩过一次，说明"数据/格式分离"还需再固化）
2. **把 build + write 用 `+` 连成一行** → `build()` 返回 dict、`write()` 返回 `None`，`dict + None` = `TypeError`；且同一行 `content` 自引用未定义。两步必须**分开写**
3. **批量解包 4 个变量但元组是 5 个** → Day 11 加了 `frequent_errors`，遗漏未同步 → `ValueError: too many values to unpack`
4. **单文件分支误用 `file['...']`** → `file` 是**批量循环**的临时变量，单文件应用 `content['...']` → `NameError`
5. **单文件 0 行时漏包 `else`** → 报告生成段在 `else` 外，`lines` 为空时仍往下跑引用未定义的 `counter` → `NameError`
6. **`Redis` 同时留在两个类别** → 因"首个匹配先赢"、数据库排在前，选 B 却没生效；**要删掉数据库里的 Redis 才真正生效**
7. **`--format` 写了手写校验而非 `choices`**、且 `default` 误设为 `html` → 偏离 Day 10「默认 txt 向后兼容」设计

### Day 13 收尾四问

1. **今天产出了什么？** → 修复 HTML 回归（含整合高频错误）+ 2 项分类覆盖修复 + 0 行友好提示 + HTML 空表头优化；全部通过 9 组测试；`log_analyzer.py` 改动待提交
2. **跑通了吗？** → ✅ 单文件/批量 HTML、默认 txt、非法值安检、0 行边界、空/异常/不存在边界、高频渲染全部通过
3. **卡在哪了？** → ① `build_*` 又写成写文件（职责错位）② 一行 `+` 拼接致 `dict+None` 崩溃 ③ 元组解包数量没跟上 Day 11 改动 ④ 单文件分支混用 `file` ⑤ 0 行漏包 `else` ⑥ Redis 双类别导致选 B 不生效
4. **到布卢姆第几层了？** → 应用层（补回 5 个函数 + 主入口分流）✅ / 分析层（看懂"数据层 dict → 格式层 writer"与"首个匹配先赢"的优先级）✅ / 评价层（判断 4 列数据该用专用渲染函数、判断 `Redis` 归类、判断 0 行提示该放主入口）✅ / **创造层**（补回归时主动整合 Day 11 高频错误，并新增 `_render_frequent_errors_table` 复用；空表头改为友好段落，两格式层行为对齐）✅

### 一句话说清今天最重要的概念

> Day 13 的价值不是"把功能加回来"，而是**用 Git 取证把丢失的增量精准合并进来、修复过程中不再制造新坑**——"修回归"最怕的不是修不好，而是"按下葫芦浮起瓢"：补回 HTML 却丢了高频错误、修好逻辑却引入 `NameError`。**改前想清职责与数据流，改后回读与逐路径验证**，才是收尾阶段真正的功课。

### Git 提交

```
Day 13: 修复Day12发现的5项问题（HTML回归+分类覆盖+0行提示）（7d5cfdc）
```
